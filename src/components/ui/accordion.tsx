...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex flex-1 items-center justify-between min-h-[48px] py-5 px-2 font-medium transition-all hover:underline [&[data-state=open]>svg]:rotate-180 text-base",
        className,
      )}
      {...props}
    >
...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Content
    ...
    {...props}
  >
    <div className={cn("pb-6 pt-2 px-2", className)}>{children}</div>
  </AccordionPrimitive.Content>
));
