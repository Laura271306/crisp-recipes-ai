...
export const reducer = (state: State, action: Action): State => {
  switch (action.type) {
    ...
    case actionTypes.DISMISS_TOAST: {
      ...
      // ! Side effects ! - This could be done in an effect instead, but for now we'll skip it
      if (toastId) {
...
export function useToast() {
  // 1. Initialize local state with the current global state value
  ...
  }, []) // Empty dependency array ensures subscription runs only once
